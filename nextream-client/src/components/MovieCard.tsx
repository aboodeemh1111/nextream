...
const MovieCard = ({
  ...
  useEffect(() => {
    ...
    if (isHovered && videoRef.current && movie.trailer) {
      ...
      const timer = setTimeout(() => {
        if (!isHovered) return; // user left before timer fired
        if (
          videoRef.current &&
          movie.trailer &&
  ...
  const toggleMyList = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (onListChange)
        onListChange(movie._id, "mylist", isInMyList ? "remove" : "add");
    } catch (err) {
  ...
  const toggleFavorites = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      // Notify parent component if needed
      if (onListChange)
        onListChange(movie._id, "favorites", isInFavorites ? "remove" : "add");
    } catch (err) {
  ...
  const toggleWatchlist = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      // Notify parent component if needed
      if (onListChange)
        onListChange(movie._id, "watchlist", isInWatchlist ? "remove" : "add");
    } catch (err) {
        ...
        {isHovered && (
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-black/60 to-black flex flex-col p-2 sm:p-3">
            <div className="flex-1 flex flex-col">
              <h3 className="text-white font-semibold text-xs sm:text-sm md:text-base mb-0.5 sm:mb-1 line-clamp-1">
                {movie.title}
              </h3>
              ...
              <p className="text-gray-300 text-xs line-clamp-2 mb-1 sm:mb-2 hidden sm:block">
                {movie.desc}
              </p>
              ...
              <div className="flex space-x-1 sm:space-x-2 mb-1 sm:mb-2">
                ...
                <button
                  ...
                  } p-1 rounded-full hover:bg-gray-700 text-xs sm:text-sm`}
