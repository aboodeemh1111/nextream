...
import { FaEdit, FaTrash, FaArrowLeft, FaPlay, FaFilm, FaCalendar, FaClock, FaTag, FaShieldAlt } from 'react-icons/fa';
    ...
    return (
      <AdminLayout>
        <div className="p-6">
          ...
          <Link href="/movies" className="text-blue-600 hover:underline flex items-center">
          ...
          <div className="relative h-64 md:h-96">
            ...
            <div className="absolute bottom-0 left-0 p-6">
              ...
              <div className="flex flex-wrap items-center text-white gap-4">
                ...
                <div className="flex items-center">
                  <FaFilm className="mr-1" /> {movie.isSeries ? 'Series' : 'Movie'}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div>
                ...
                <div className="space-y-4">
                  {movie.imgSm && (
                    <div>
                      <h3 className="text-sm font-medium text-gray-500 mb-2">Thumbnail Image</h3>
                  ...
                  {movie.imgTitle && (
                    <div>
                      <h3 className="text-sm font-medium text-gray-500 mb-2">Title Image</h3>
                      <div className="relative h-40 bg-gray-100 rounded-md overflow-hidden">
                        <Image
                          src={movie.imgTitle}
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-gray-500 mb-2">Storage URLs</h3>
                    <div className="space-y-2">
                      {movie.img && (
                        <div className="text-xs text-gray-500 truncate">
                          <span className="font-semibold">Main Image:</span> {movie.img}
                        </div>
                      )}
                      {movie.imgTitle && (
                        <div className="text-xs text-gray-500 truncate">
                          <span className="font-semibold">Title Image:</span> {movie.imgTitle}
                        </div>
                      )}
                      {movie.imgSm && (
                        <div className="text-xs text-gray-500 truncate">
                          <span className="font-semibold">Thumbnail:</span> {movie.imgSm}
                        </div>
                      )}
                      {movie.trailer && (
                        <div className="text-xs text-gray-500 truncate">
                          <span className="font-semibold">Trailer:</span> {movie.trailer}
                        </div>
                      )}
                      {movie.video && (
                        <div className="text-xs text-gray-500 truncate">
                          <span className="font-semibold">Video:</span> {movie.video}
                        </div>
                      )}
                    </div>
                  </div>
